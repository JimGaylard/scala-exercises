...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def upsert(dic: Map[Char, Int], char: Char) = {
      val current = dic.getOrElse(char, 0)
      dic +((char, current + 1))
    }
    chars.foldLeft(Map[Char, Int]())(upsert).toList
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def lookup(tree: CodeTree)(c: Char): List[Bit] = tree match {
      case Leaf(_,_) => Nil
      case Fork(l, r, _, _) =>
        if (chars(l).contains(c)) 0 :: lookup(l)(c)
        else 1 :: lookup(r)(c)
    }
